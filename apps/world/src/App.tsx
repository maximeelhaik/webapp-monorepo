...
export default function App() {
  ...
  // Suggérer des mots par défaut
  const suggestions = ['Bonjour', 'Merci', 'Amour', 'Liberté', 'Sourire', 'Voyage'];
  ...
  }, [placeholder, isDeleting, wordIndex, word]);

  // Appel de l'API avec streaming
  const handleGenerate = async (presetWord?: string) => {
    const currentWord = suggestions[wordIndex];
    const wordToUse = presetWord || word || currentWord;
    ...
    console.log(`[App ${getTime()} - +0ms] ✉️ Envoi de la requête pour le mot: "${wordToUse}"`);

    try {
      const response = await fetch('/api/generate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ prompt: wordToUse, app: 'world' }),
      });
      ...
      while (!done) {
        ...
        if (chunkValue) {
          if (!firstTokenReceived && chunkValue.trim().length > 0) {
            ...
            console.log(`[App ${getTime()} - +${elapsed()}] ⏱️ Token reçu: "${chunkValue.replace(/\n/g, '\\n')}"`);
          }
  ...
  // Convertir le texte brut généré à la volée en un tableau de traductions
  const translations = aiResponse
    .trim()
    .split(/[|\n]/)
    .map((line) => line.trim())
    .filter((line) => {
  ...
  return (
    ...
    >
      ...
      <header className="mb-12 text-center select-none w-full max-w-4xl mx-auto">
        ...
        >
          Traduisez instantanément vos mots dans 20 langues différentes.
        </motion.p>
      </header>
